...
install-solr:
	cd ~/src && \
		sudo -u allura wget -nv http://archive.apache.org/dist/lucene/solr/5.3.1/solr-5.3.1.tgz && \
		sudo -u allura tar xvf solr-5.3.1.tgz solr-5.3.1/bin/install_solr_service.sh --strip-components=2 && \
		sudo -u allura sudo ./install_solr_service.sh solr-5.3.1.tgz
	cd ~/src/allura && \
